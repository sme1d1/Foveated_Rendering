...
sparsenp = sparsenp.astype('float32') / 255.
x_train = x_train.reshape((len(x_train), np.prod(x_train.shape[1:])))
sparsenp = sparsenp.reshape((len(sparse), np.prod(sparsenp.shape[1:])))
...
history = autoencoder.fit(x_train, x_train,
                          ...
                          shuffle=True,
                          verbose=2,
                          validation_data=(sparsenp, sparsenp))

# Create predictions

encoded_imgs = encoder.predict(sparsenp)
...
for i in range(n):
    ...
    plt.imshow(sparsenp[i].reshape(rowcol, rowcol))
    plt.gray()
