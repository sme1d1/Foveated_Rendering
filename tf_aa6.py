...
#np.load(os.path.join(savepath, 'catscolor.npy'))
...
for i in range(Xlen):
    pic = sparse(X[i], a)
    plt.imsave('./images/savesparse/sparse_%i.png'%i, pic)

savepath = './imgdb'
path_sparse = './images/savesparse'
sparse_data = []
# for generating our dataset
...
x_train1, x_test = train_test_split(Z, test_size=0.25, random_state=42)
x_train, x_test1 = train_test_split(X, test_size=0.25, random_state=42)
...
encoding_dim = 3072  # 32 floats -> compression of factor 24.5, assuming the input is 784 floats
# this is our input placeholder
input_img = Input(shape=(image_size_squared,))

# "encoded" is the encoded representation of the input

hidden_layer1 = Dense(1024, activation='relu')(input_img)  # add hidden layer
hidden_layer = Dense(512, activation='relu')(hidden_layer1)  # add hidden layer
